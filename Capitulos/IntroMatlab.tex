...
\noindent Los comentarios van con signo de porcentaje \texttt{\%}. Para no mostrar la asignación de la variable en la ventana de comandos, escribir el símbolo \texttt{;} al final de la asignación.
...
\begin{itemize}
    \item \texttt{log()} y \texttt{exp()} - Logaritmo natural y exponencial
    \item \texttt{sqrt()} y \texttt{()\^ \ x} - Raíces y Potencias
    \item \texttt{sin()} y \texttt{cos()} - Seno y coseno
    \item $x = A\setminus B$ - Solución de $Ax = B$
    %\item $x = B/A$ - Solución de $X*A = B$
    \item \texttt{diag()} - de vector a matriz diagonal o extraer diagonal de una matriz
\end{itemize}
...
\noindent Notar que la variable tipo sistema tiene toda la información del sistema. Para mostrarla se puede escribir $sys$ en la ventana de comandos o $disp(sys)$
...
\noindent Si se desea extraer la información del sistema que relaciona el output $i$ con el input $j$, se puede utilizar $sys(i,j)$
\begin{verbatimg}
    bode(sys(i,j)) % grafica sólo el bode (i,j)
    impulse(sys(i,j)) % grafica la respuesta del output "i" cuando input "j" es un impulso
\end{verbatimg}
