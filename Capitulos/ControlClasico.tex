...
Cuando se añade un dispositivo de control al sistema dinámico estructural, se genera un nuevo sistema dinámico con propiedades distintas. Si se desea encontrar la función de transferencia ($H(s)$) del nuevo sistema $Y(s) = H(s)R(s)$, donde $R(s)$ es la entrada (excitación) e $Y(s)$ es la salida (respuesta), se puede obtener aplicando la transformada de laplace a la EDM del nuevo sistema. Sin embargo es mucho más rápido encontrar $H(s)$ utilizando el diagrama de bloques si es que se saben a priori las funciones de transferencia tanto del controlador (actuador) como de la planta (edificio).

Consideremos el siguiente ejemplo de diagrama de bloque:
\begin{figure}[H]
\centering
...
\noindent pero, la señal de entrada $U(s)$ es la señal de salida del controlador:
\begin{align*}
    U(s) = C(s)E(s)
\end{align*}

\noindent reemplazando, se obtiene que :
\begin{align*}
    Y(s) = G(s)C(s)E(s)
\end{align*}
