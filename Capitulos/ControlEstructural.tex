...
Los sistemas de control estructural son dispositivos mecánicos que se incorporan a una estructura para poder mejorar su desempeño y serviciabilidad ante cargas dinámicas mediante reducción de la respuesta y pérdidas por daños en elementos estructurales y no estructurales. \\

Estos dispositivos se han clasificado en cuatro grupos en función de cómo operan, los grupos son: (i) pasivos, (ii) activos, (iii) semi-activos e (iv) híbridos.

\section{Control pasivo}
Los sistemas de control pasivo no requieren de una fuente de energía externa y funcionan absorbiendo, disipando o aislando energía. Su operación puede depender tanto de el desplazamien, velocidad y aceleración relativa entre los nodos de conexión del dispositivo. Algunos de estos dispositivos requieren ser reemplazados luego de ser utilizados. \\

La ecuación general para un sistema estructural con dispositivos de control pasivo es:
\begin{align*}
    m\ddot{x} + c\dot{x} + kx + fp(x,\dot{x},\ddot{x}) = \cancelto{0}{f_a} + f_e
\end{align*}

donde $f_a$: es la fuerza de un actuador (en control pasivo no hay actuadores) y $f_e$: fuerza externa. $f_p$ varía en función de si la operación del dispositivo depende del desplazamiento, velocidad o aceleración. \\
...
\begin{itemize}
    ...
    \item TMD: $f_p = m_{T} \ddot{x}_{rT}$
\end{itemize}

\subsection{Dispositivos de Disipación de Energía}
\subsubsection{Dispositivos Histeréticos}

\underline{Dispositivos Metálicos}:
Para modelar la fuerza de los BRB .... 

\underline{Dispositivos de Fricción}
Para modelar la fuerza de los dispositivos de fricción (modelo de Coulomb de fricción):
\begin{align*}
    P(t) = \mu N(t) \text{sgn}(\dot{u}(t))
\end{align*}

\noindent donde $P(t)$ es la fuerza que aplica el dispositivo a la estructura, $\mu$ es el coeficiente de fricción (roce) dinámico, $N(t)$ es la fuerza normal en la superficie de deslizamiento y $\dot{u}$ es la velocidad relativa del deslizamDiento de las superficies.
...
\subsection{Dispositivos de Aislación Sísmica}
% Saaed
